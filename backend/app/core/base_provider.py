...
from app.schemas.provider import ProviderConfig, ProviderDurationResolution, ProviderModel, ProviderModelType
...
def _build_klingai_video_model(
    ...
    return ProviderModel(
        ...
        voices=[],
        audio="off",
        duration_resolution_map=[ProviderDurationResolution(duration=durations, resolution=resolutions)],
...
def _infer_model_type(model_id: str) -> ProviderModelType:
...
def _read_provider_config_from_source(source: str, path: Path) -> ProviderConfig:
    ...
    if node.value is None:
        raise ProviderValidationError(f"{path.name} 中的 PROVIDER_CONFIG 缺少赋值")
    try:
...
class BaseProvider:
    """大模型服务工具类基类，provider 文件中的工具类必须继承此类。"""

    provider_key: str = ""
    provider_config: dict[str, Any] = {}
    model_type: str = ""

    async def generate(self, *, model_id: str | None = None) -> Any:
