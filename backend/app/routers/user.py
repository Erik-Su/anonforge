...
class UserView(BaseView):
    ...
    async def list_users(self, session: SessionDep) -> list[User]:
        """获取用户列表。
    ...
    async def get_current_user(self, request: Request, session: SessionDep) -> User:
        """获取当前登录用户资料。"""
    ...
    async def get_user_detail(self, public_id: str, session: SessionDep) -> User:
        """获取用户详情。
    ...
    async def create_user(self, payload: UserCreate, session: SessionDep) -> User:
        """创建用户。
    ...
    async def update_user(self, public_id: str, payload: UserUpdate, session: SessionDep) -> User:
        """更新用户信息。
    ...
    async def disable_user(self, public_id: str, session: SessionDep) -> User:
        """禁用用户。
