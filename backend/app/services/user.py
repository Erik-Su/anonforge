...
async def list_users(session: AsyncSession) -> list[User]:
    """获取用户列表。"""
    statement = select(User).order_by(col(User.sort_order), col(User.id))
    result = await session.exec(statement)
...
async def ensure_default_admin() -> None:
    ...
    async with get_session_maker()() as session:
        result = await session.exec(select(User.id).where(col(User.is_superuser).is_(True)))
        ...
        now = utc_now()
        admin_user = User.model_validate({
