...
async def _get_project_by_public_id(
    ...
    statement = select(Project).where(col(Project.public_id) == public_id)
    result = await session.exec(statement)
    return result.first()
...
async def search_projects_by_name(
    ...
    keyword = name.strip()
    if keyword:
        statement = statement.where(col(Project.name).ilike(f"%{keyword}%"))
...
async def search_projects_by_member(
    ...
    statement = select(Project).where(
        or_(
            col(Project.owner_id) == member_public_id,
...
async def search_project_member_candidates(
    ...
    statement = (
        ...
        .where(
            or_(
                col(User.username).ilike(f"%{search_keyword}%"),
                col(User.email).ilike(f"%{search_keyword}%"),
            ),
            col(User.public_id) != project.owner_id,
            ~exists().where(
                col(ProjectMember.project_id) == project.id,
                col(ProjectMember.user_public_id) == col(User.public_id),
            ),
        )
        .order_by(col(User.sort_order), col(User.id))
    )
    result = await session.exec(statement)
...
async def _get_project_member(
    ...
    statement = select(ProjectMember).where(
        ...
        col(ProjectMember.user_public_id) == user_public_id,
    )
    result = await session.exec(statement)
    return result.first()
...
async def _list_project_members_by_project_id(
    ...
    statement = (
        ...
        .where(col(ProjectMember.project_id) == project_id)
        .order_by(col(ProjectMember.id))
    )
    result = await session.exec(statement)
    return list(result.all())
...
async def _list_project_member_reads_by_project_id(
    ...
    statement = (
        ...
        .join(User, col(ProjectMember.user_public_id) == col(User.public_id), isouter=True)
        .where(col(ProjectMember.project_id) == project_id)
        .order_by(col(ProjectMember.id))
    )
    result = await session.exec(statement)
...
async def _get_current_user_or_raise(session: AsyncSession, current_user_public_id: str) -> User:
    ...
    statement = select(User).where(col(User.public_id) == current_user_public_id)
    result = await session.exec(statement)
    user = result.first()
    if user is None:
...
async def _get_user_by_public_id_or_raise(session: AsyncSession, public_id: str) -> User:
    """获取用户，不存在时按成员不存在处理。"""
    statement = select(User).where(col(User.public_id) == public_id)
    result = await session.exec(statement)
    user = result.first()
    if user is None:
...
async def _list_accessible_projects(
    ...
    statement = statement.distinct().order_by(col(Project.sort_order), col(Project.id))
    result = await session.exec(statement)
    return list(result.all())
...
def _project_related_to_user_condition(user_public_id: str):
    """返回项目与用户相关的 SQL 条件。"""
    return or_(
        col(Project.owner_id) == user_public_id,
...
def _project_has_member_condition(user_public_id: str):
    """返回项目包含指定成员的 SQL 条件。"""
    return exists().where(
        col(ProjectMember.project_id) == col(Project.id),
        col(ProjectMember.user_public_id) == user_public_id,
    )
